...
describe('CTEs and Subqueries', () => {
  ...
  describe('Common Table Expressions (CTEs)', () => {
    it('compiles a single WITH clause', async () => {
      ...
      expect(sql).toContain(
        'WITH "active_accounts" AS (SELECT * FROM "accounts" WHERE "status" = @p0)',
      );
    ...
    it('compiles a WITH RECURSIVE clause', async () => {
      ...
      expect(sql).toContain(
        'WITH RECURSIVE "org_tree" AS (SELECT * FROM "employees" WHERE "managerId" IS NULL)',
      );
    });
    ...
    it('compiles multiple chained CTEs', async () => {
      ...
      const query = ordersSet.withCte('active_acc', q1).withCte('rich_acc', q2);

      await query.toList();

      const { sql } = adapter.executedQueries[0];
      expect(sql).toContain(
        'WITH "active_acc" AS (SELECT * FROM "accounts" WHERE "status" = @p0), "rich_acc" AS (SELECT * FROM "accounts" WHERE "balance" = @p1) SELECT * FROM "orders"',
      );
    });
  });
  ...
  describe('Subqueries and Existence Predicates', () => {
    ...
    it('compiles whereExists with dynamic proxy field join predicate', async () => {
      const highValueCustomers = accountsSet.where(w => w.gt('balance', 100000)).asSubquery('hvc');
      ...
      const { sql, params } = adapter.executedQueries[0];
      expect(sql).toContain(
        'WHERE EXISTS (SELECT 1 FROM "accounts" AS "hvc" WHERE "balance" > @p0 AND ("orders"."customerId" = "hvc"."id"))',
      );
    ...
    it('compiles whereNotExists correctly', async () => {
      ...
      const { sql } = adapter.executedQueries[0];
      expect(sql).toContain(
        'WHERE NOT EXISTS (SELECT 1 FROM "accounts" AS "sub_acc" WHERE ("orders"."customerId" = "sub_acc"."id"))',
      );
    });
    ...
    it('supports whereExists with fluent WhereClause callback', async () => {
      ...
      const { sql } = adapter.executedQueries[0];
      expect(sql).toContain(
        'WHERE EXISTS (SELECT 1 FROM "accounts" AS "hvc" WHERE "status" = @p0)',
      );
    });
  });
});

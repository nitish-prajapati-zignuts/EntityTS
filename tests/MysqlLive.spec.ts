...
// ─── 1. Entities for MySQL Live Testing ─────────────────────────────────────
...
class MysqlDept {
  ...
  @HasMany(() => MysqlEmployee, 'deptId')
  employees?: MysqlEmployee[];
}
...
class MysqlEmployee {
  ...
  @BelongsTo(() => MysqlDept, 'deptId')
  dept?: MysqlDept;
}

// ─── 2. MySQL Live Context ──────────────────────────────────────────────────

const mysqlUrl = process.env.MYSQL_URL || process.env.DATABASE_URL_MYSQL;
const describeMysql = mysqlUrl ? describe : describe.skip;

class MysqlLiveContext extends DbContext {
  public accounts = this.set(MysqlAccount);
  public depts = this.set(MysqlDept);
  public employees = this.set(MysqlEmployee);
...
describeMysql('MySQL Live Integration & Feature Specs', () => {
  let ctx: MysqlLiveContext;

  beforeAll(async () => {
    ctx = new MysqlLiveContext();
  ...
  it('verifies MySQL connection pool health', async () => {
  ...
  it('executes CRUD with optimistic concurrency control on MySQL', async () => {
    // 1. Add
  ...
  it('executes UnitOfWork pattern with topological dependency resolution on MySQL', async () => {
    const uow = ctx.createUnitOfWork();

    const dept = new MysqlDept();
    dept.id = 100;
    dept.name = 'Engineering';
    dept.version = 1;
    uow.registerNew(ctx.depts, dept);

    const emp = new MysqlEmployee();
    emp.id = 501;
    emp.deptId = 100;
  ...
  it('cascades soft deletes and restorations recursively on MySQL', async () => {
  ...
  it('rolls back MySQL transactions on aborted operations', async () => {
    await expect(

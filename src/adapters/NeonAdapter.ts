...
/**
 * Neon Serverless PostgreSQL adapter using `@neondatabase/serverless`.
 * Supports serverless connection pooling via WebSockets and HTTP.
 */
export class NeonAdapter implements IDbAdapter {
  public readonly provider: DbProvider = 'neon';
  private neonModule: any;
  private pool: any;

  constructor(private readonly config: NeonAdapterConfig | string) {}
  ...
  public async executeQuery<T = unknown>(
    ...
    try {
      ...
      throw new QueryException(`Failed to execute Neon query: ${(err as Error).message}`, sql, err);
    }
  }
  ...
  private async resolveNeon(): Promise<any> {
    const { loadDriver } = await import('./DriverLoader');
    return await loadDriver('neon', '@neondatabase/serverless');
  }
}
